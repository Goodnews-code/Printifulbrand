...
/** Customer logo uploads for package customizations (public API). */
export async function uploadPackageLogo(file: File) {
  const supabase = getSupabaseAdmin();
  ...
  try {
    ...
    console.error("[storage] package logo optimize failed, using original:", err);
    const ext = extensionFromNameOrMime(file);
    optimized = {
      buffer: input,
      contentType: file.type || "image/jpeg",
  ...
  const filename = `package-logos/logo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${optimized.extension}`;
